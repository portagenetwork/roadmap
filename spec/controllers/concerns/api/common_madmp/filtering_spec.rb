...
RSpec.describe Api::CommonMadmp::Filtering do
  ...
  describe '#apply_filters' do
    ...
    context 'with a scalar-only key passed as a nested hash' do
    ...
    context 'with a date key passed as an array' do
      let(:params) { { created_after: %w[2024-01-01 2024-02-01] } }
    ...
    context 'when one key is array-shaped and another is a valid scalar filter' do
      let(:params) { { title: %w[Alpha Beta], created_after: '2024-01-01' } }

      it 'rejects the whole request rather than applying only the valid filter' do
        instance.send(:apply_filters, scope)
