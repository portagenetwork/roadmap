  ...
  describe '#apply_sorting' do
    context 'when no sort param is given' do
      it 'applies the default sort (created desc)' do
        ...
        expect(scope.order_calls).to eq([{ 'plans.created_at' => :desc }])
      end
    end
    ...
    context 'with a single valid sort param' do
      ...
      it 'maps the field to its qualified column and applies order' do
        instance.send(:apply_sorting, scope)

        expect(scope.order_calls).to eq([{ 'plans.title' => :asc }])
      end
    end

    context 'with a valid language sort param' do
      let(:params) { { sort: ['language,asc'] } }

      it 'maps language to the qualified language column and applies order' do
        instance.send(:apply_sorting, scope)

        expect(scope.order_calls).to eq([{ 'plans.language' => :asc }])
      end
    end
    ...
    context 'with multiple valid sort params' do
      ...
      it 'chains order calls for each field, in order' do
        ...
        expect(scope.order_calls).to eq([{ 'plans.title' => :asc }, { 'plans.created_at' => :desc }])
  ...
  describe '#parse_sort_params' do
    context 'when sort is blank' do
      ...
      it 'falls back to the default sort' do
        expect(instance.send(:parse_sort_params)).to eq([['plans.created_at', :desc]])
      end
    end
    ...
    context 'with surrounding whitespace and mixed case direction' do
      ...
      it 'strips whitespace and downcases the direction' do
        expect(instance.send(:parse_sort_params)).to eq([['plans.title', :asc]])
      end
    end
