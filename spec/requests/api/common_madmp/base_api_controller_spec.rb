...
RSpec.describe Api::V2::BaseApiController do
  ...
  describe 'access logging (log_access)' do
    it 'logs the client application name, uid, and resource owner id' do
      ...
      expect(logged_messages).to include(a_string_matching(/Client \(OAuth\) application name: #{Regexp.escape(@client.name)}/))
      expect(logged_messages).to include(a_string_matching(/Client \(OAuth\) application uid: #{Regexp.escape(@client.uid)}/))
      expect(logged_messages).to include(a_string_matching(/Resource owner id: #{@user.id}/))
    end
  end
